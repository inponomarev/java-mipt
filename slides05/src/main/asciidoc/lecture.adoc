...
* По умолчанию: -128..127
* Можно поднять верхнюю границу через параметр `-XX:AutoBoxCacheMax=<size>`
* Нижнюю границу опустить нельзя :-)

== Что и как ещё кэшируется

* `Byte` -- полностью (-128..127)
* `Short`, `Long` -- -128..127
* `Character` -- 0..127
* `Boolean` -- `Boolean.TRUE` и `Boolean.FALSE`
* `Float`, `Double` -- не кэшируются, хотя в контракте `valueOf` прописана возможность кэширования часто используемых значений.
...
* `long System.nanoTime()` -- при замерах периодов времени. 
...
* `Period`: разица во времени между `LocalDate` в днях (например, "сколько дней действует договор?")
