...
* Одна программа -- много параллельно выполняющихся тредов (threads)
...
* &beta; -- параметр, определяющий cohesion
...
[source,java]
----
static class CalcSquare extends Thread {
    final int argument;
    int result;
...
* Чтобы действие B гарантированно видело результат работы действия A, необходимо, чтобы stem:[A \prec B].
...
* Благодаря этому правилу, наш самый первый пример на параллелизацию работает корректно.
...
* Это автоматом делает видимыми изменения в других переменных, но полагаться на это _настоятельно_ не рекомендуется.
...
(кстати, `volatile array` --- не массив `volatile`-элементов)
|===
...
* Разблокировка замка _happens-before_ другой блокировки того же замка.
* Поэтому защищённые замком переменные объявлять как `volatile` уже не нужно.
...
== Часть 2. Потокобезопасные структуры данных
...
* В ранних версиях Java можно было «сделать» коллекцию потокобезопасной, обернув в `Collections.synchronizedXXX(...)`. Это сериализовывало любой доступ к внутреннему состоянию коллекции.
...
* В стандартной библиотеке для этого есть Thread Pools, не надо ничего делать самому.
...
* Если вычисления в цикле, тред обязан периодически проверять статус `isInterrupted()` и выходить из метода, если прервалось.
* На ждущих методах может быть выброшен `InterruptedException`, который следует либо пробросить выше, либо 
...
----
* "Проглатывать" `InterruptedException` *ни в коем случае нельзя*!
...
* Каждый иэ этих методов имеет вариант с постфиксом `Asinc` (например, `thenApplyAsync`), позволяющий выполнить дополнительное действие в другом треде заданного Executor-a.
