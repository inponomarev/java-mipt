...
== Создание Connection, Statement, получение ResultSet и итерация по ResultSet

[plantuml, sequence, png]
----
skinparam dpi 150
client->DriverManager : getConnection()
DriverManager-->client: connection

client->connection: createStatement()
connection-->client: statement

client->statement: executeQuery()
statement-->client: resultSet
...
То, что написано во всех тьюториалах и учебниках, и то, что вы *никогда* не будете делать в реальной жизни:
[source,java]
----
...
pooledConnection->pool: add()
----

== Connection Pool

* Грамотно реализовать самостоятельно -- сложно
* Предоставляет ваш фреймворк / контейнер / JDBC driver
