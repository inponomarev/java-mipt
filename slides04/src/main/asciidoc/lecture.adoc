...
* Оберните в unchecked exception (`InvalidStateException`, например), с помощью `initCause`.
* Lobmok's `@SneakyThrows` (самая спорная фича Lombok)
...
String name3 = new String("Jane Roe");
----
^.^a|
