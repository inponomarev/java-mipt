...
# The classic GitHub theme, as shipped by Rouge up to and including 3.30.0.
# Rouge 4.0.0 replaced `Rouge::Themes::Github` with a variant based on GitHub's
# Primer palette (bright red/purple/orange, no bold), which reads poorly on our
# light slide background. We need Rouge >= 4.5.2 for the `record` keyword, so the
# old theme is vendored here and selected via the `rouge-style` attribute.

require 'rouge'

module Rouge
  module Themes
    class GithubClassic < CSSTheme
      name 'github-classic'
