...
//Вариант: сериализуем по умолчанию, помечаем то, что сериализовывать не хотим.
public class Person {
    @Publish
...
** На этапе кодогенерации перед компиляцией (annotation processing)
...
* `String`,
* `Class` (с возможным типовым параметром, вроде `Class<? extends MyClass`),
* другие аннотации,
...
//Если на параметр не задан default, его определение будет обязательно
...
//...Как видим, используя примитивы, можно задать сколь угодно сложную структуру данных
----
...
//и т. д., список не полный. В общем, аннотации могут быть почти везде
----
...
== Стандартные аннотации, встроенные в язык

Уже знакомые аннотации:
[cols="20a,20a,60a"]
|===
|*Annotation +
interface*
...
| `PACKAGE`         | Пакеты
| `TYPE`            | Любые классы (включая `enums`) и интерфейсы (включая интерфейсы аннотаций)
| `METHOD`          | Методы
...
[cols="30a,70a"]
|===
| *Retention Policy* | *Доступность аннотаций*
| `SOURCE` 
| На уровне исходников для annotation processors. Не попадают в class-файлы и runtime.
| `CLASS` + 
(по умолчанию)
| На уровне исходников и class-файлов. Доступны для instrumentation API. Невидимы в runtime.
| `RUNTIME`
| На уровне исходников, в class-файлах и во время исполнения. Доступны для Reflection API.
|===
...
* Литералы -- строки в коде, порождающие объекты. Например, `"foo"` -- литерал, порождающий объект `String` с содержимым "foo".
...
//Тут ничего непонятного нет, массивами можно параметризовать:
...
== JsonSerializer: определение полей, помеченных @Publish
