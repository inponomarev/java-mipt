...
** В стандартной библиотеке:  `Stream<Integer>`/`<Double>`/`<Long>` -> `IntStream`/`DoubleStream`/`LongStream`.
** В специализированных библиотеках вроде http://fastutil.di.unimi.it/[fastutil]: `ArrayList<Integer>` -> `IntArrayList`, `HashMap<Integer, V>` -> `Int2ObjectMap<V>` (ВНИМАНИЕ: реальная потребность в таких библиотеках возникает редко!!) 
...
"List<? extends Employee>" <|-- "List<Employee>"

"List<? extends Employee>" <|-- "List<Manager>"
