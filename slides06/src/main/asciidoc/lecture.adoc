...
b.add("manager");
//Выполняется: мы list.get вернул Object
...
Нужна производительность? Пишем специальные реализации.
...
== Решается с помощью метакласса и рефлекции (о которой речь впереди)
...
StringHolder h = new StringHolder().setValue("aaa");
----
